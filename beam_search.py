...
class Beam(object):
    ...
    def advance(self, prob_dist, hidden_state, context, sum_temporal_srcs, prev_s):
        ...
        best_scores, best_scores_id = T.topk(input=scores, k=config.beam_size,
    ...
    def get_best(self):
        best_token = self.tokens[
            0].cpu().numpy().tolist()  # Since beams are always in sorted (descending) order, 1st beam is the best beam
        try:
...
def beam_search(enc_hid, enc_out, enc_padding_mask, ct_e, extra_zeros, enc_batch_extend_vocab, model, start_id, end_id,
    ...
    for t in range(config.max_dec_steps):
        x_t = T.stack(
            [beam.get_current_state() for beam in beams if beam.done is False]  # remaining(rem),beam
        ).contiguous().view(-1)  # (rem*beam,)
        x_t = model.embeds(x_t)  # rem*beam, n_emb

        dec_h = T.stack(
            [beam.hid_h for beam in beams if beam.done is False]  # rem*beam,n_hid
        ).contiguous().view(-1, config.hidden_dim)
        dec_c = T.stack(
            [beam.hid_c for beam in beams if beam.done is False]  # rem,beam,n_hid
        ).contiguous().view(-1, config.hidden_dim)  # rem*beam,n_hid

        ct_e = T.stack(
            [beam.context for beam in beams if beam.done is False]  # rem,beam,n_hid
        ).contiguous().view(-1, 2 * config.hidden_dim)  # rem,beam,n_hid

        if sum_temporal_srcs is not None:
            sum_temporal_srcs = T.stack(
                [beam.sum_temporal_srcs for beam in beams if beam.done is False]
            ).contiguous().view(-1, enc_out.size(1))  # rem*beam, n_seq

        if prev_s is not None:
            prev_s = T.stack(
                [beam.prev_s for beam in beams if beam.done is False]
